...
def load_data() -> dict[str, np.ndarray]:
	"""
	Loads preprocessed coverage features and labels from disk.

	Inputs:
	- None (uses PROJECT_ROOT paths).
...
def plot_roc(y_test: np.ndarray, y_proba: np.ndarray) -> None:
	"""
...
def model_man_vs_zone() -> None:
	"""
	Trains and evaluates an XGBoost model for man vs. zone coverage.

	Inputs:
	- None (loads data from disk and uses fixed params).
	...
	with mlflow.start_run(run_name="xgb-10fold"):
		...
		mlflow.log_params({"cv_folds": 10, "test_size": 0.2, "shuffle": True, "random_state": 42})
		...
		# Train
		train_xgb(model=xgbModel, skf=skf, x_train=x_train_scaled, y_train=y_train, x_test=x_test_scaled, y_test=y_test)
