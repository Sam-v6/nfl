...
def set_seed(seed: int = 42, deterministic: bool = False) -> torch.Generator:
        ...
        - deterministic: Whether to enforce fully deterministic Torch behavior.
        ...
        if deterministic:
                # Enforce deterministic algorithms and disable sources of randomness
                os.environ["CUBLAS_WORKSPACE_CONFIG"] = ":4096:8"
                torch.use_deterministic_algorithms(True)
                torch.backends.cudnn.deterministic = True
                torch.backends.cudnn.benchmark = False
                # Avoid TF32 for bitwise repeatability
                torch.backends.cuda.matmul.allow_tf32 = False
                torch.backends.cudnn.allow_tf32 = False
        else:
                # Allow fast algorithms with slight nondeterminism risk
...
def run_trial(config: dict[str, float | int], args: argparse.Namespace) -> None:
        ...
        amp_dtype = torch.float16 if not deterministic_mode else torch.float32
        ...
        ######################################################################
        # Create transformer with given config, move to GPU, and optionally compile model for better inference (will save as compiled model)
        ...
        optimizer = torch.optim.AdamW(
                ...
                fused=not deterministic_mode,  # Fused kernels are faster but can be nondeterministic
        )
