...
def _determine_min_frames(plays_df, location_data_df) -> tuple[np.array, np.array]:
...
def _get_last_frame(plays_df, location_data_df):

    min_frame_start = time.time()
    logging.info("Determining last frame of play...")
    frame_length = []
    ...
    for i in range(0, len(plays_df)):
        ...
        # Skip play if there aren't 11 defenders for some reason (Ole Billy or Vrabel filtering here)
        if location_play_df['nflId'].nunique() != 22:
...
def create_coverage_data(games_df, plays_df, players_df, location_data_df) -> None:
    ...
    logging.info(f'Total plays after filtering out penalties: {len(plays_df)}')

    plays_df = plays_df[plays_df['pff_manZone'].isin(['Man', 'Zone'])]
    logging.info(f'Total plays after filtering to valid Man or Zone classifications: {len(plays_df)}')

    plays_df = plays_df[plays_df['gameId'].isin(location_data_df['gameId'].unique())]
    logging.info(f'Total plays after matching plays_df to location_data_df: {len(plays_df)}')
    ...
    #_get_last_frame(plays_df, location_data_df)
